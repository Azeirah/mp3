...
public class ScreenElement implements Serializable {
    ...
    protected void drawScreenElements() {
        ...
        // Als je eigen elementen ook kinder elementen hebben, teken die dan ook.
        // Dit maakt het makkelijk om bijvoorbeeld een knop met een randje te maken. Dan kan de rand altijd zwart blijven
        // en dan kan je de binnenkant inverteren, bijvoorbeeld als je de knop selecteert.
    ...
    protected void drawScreenElement(ScreenElement element) {
        ...
        for (int x = 0; x < element.getWidth(); x++) {
            for (int y = 0; y < element.getHeight(); y++) {
                if (dotArray[x][y])
                    setPixel(x + element.getLocationX(), y + element.getLocationY());
            }
        }
    }
    ...
    public void fromBMPFile(File BMPFileName) throws IOException {
        ...
        for (int yPixel = 0; yPixel < image.getHeight(); yPixel++) {
            for (int xPixel = 0; xPixel < image.getWidth(); xPixel++) {
                ...
                // nice, intelliJ verteld me wanneer een statement simpeler gemaakt kan worden. lol
//                if (color == Color.BLACK.getRGB()) {
//                    _array2D[yPixel][xPixel] = true;
//                } else {
//                    _array2D[yPixel][xPixel] = false; // ?
//                }
            }
        }
        dotArray = _array2D;
    ...
    public void saveSerializedData(String filename) {
        if (loaded) {
            ...
            System.out.println("Please load a DotArray from a BMP or serialized file before trying to serialize it.");
        }
    }
